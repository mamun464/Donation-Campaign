...
const Home = () => {
    ...
    const handleSearchButton = () => {
        if (searchValue) {
            const filterData = donations.filter(donation => donation?.category?.toLowerCase() === searchValue.toLowerCase())
            setDisplayDonation(filterData)
        }
        else {
            setDisplayDonation(donations)
        }
    ...
    return (
        <>
            ...
            <div className='p-8 mt-24 max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4'>
                {
                    displayDonation.map(donation => <DonationCard
                        key={donation.id}
                        donation={donation}
                    ></DonationCard>)
                }
